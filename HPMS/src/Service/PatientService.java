...
public class PatientService {
    ...
    /** Returns all patients (including archived). Prefer listActive() for UI lists. */
    public Collection<Patient> listAll() { return repo.findAll(); }

    /** Return only non-archived patients for active listings. */
    public Collection<Patient> listActive() {
        java.util.List<Patient> out = new java.util.ArrayList<>();
        for (Patient p : repo.findAll()) if (!p.isArchived()) out.add(p);
        return out;
    }

    /** Return only archived (former) patients. */
    public Collection<Patient> listArchived() {
        java.util.List<Patient> out = new java.util.ArrayList<>();
        for (Patient p : repo.findAll()) if (p.isArchived()) out.add(p);
        return out;
    }

    /** Hard delete kept for admin tasks; use archivePatient for typical doctor-level removal. */
    public boolean deletePatient(String id) { return repo.delete(id); }

    /** Soft-archive a patient (mark archived=true). Returns true if patient found and archived state set. */
    ...
    /** Unarchive a patient (restore to active). */
    ...
    /** Update an existing patient (persist changes). Returns true when saved. */
