...
public class PatientDashboardPanel extends JPanel implements GlobalSearchable {
    ...
    private JPanel buildServicesPanel() {
        ...
        // Load announcements for the current user
        Service.HospitalService hs = Service.HospitalService.getInstance();
        Runnable reloadAnnouncements = () -> {
            annModel.clear();
            java.util.List<Service.HospitalService.Announcement> anns = hs.listAnnouncementsForUser(currentUsername==null?"":currentUsername);
            for (Service.HospitalService.Announcement a : anns) annModel.addElement(a.toString());
        };
